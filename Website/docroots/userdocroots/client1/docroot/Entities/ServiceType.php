 ...
 */
class ServiceType implements EntityBase {
    ...
    public static function GetServiceTypebyID($service) {
        try {
            $conn = DataAccess::connect();
            $srvtp = null;
            if($conn != NULL) {
                $sql = "SELECT servicetypeid, servicetypename, description, pricetag, url FROM SERVICETYPE WHERE servicetypeid='".$service."'";
